...
This module implements the query side for image-to-image retrieval. Linda's
multimodal ingestion stores webpage image embeddings in Milvus. This module
embeds a local query image with the same Nova multimodal model and searches the
same image collection.
"""
...
from common.multimodal_embeddings import NovaMultimodalEmbedder
...
def search_image(
  ...
  embedder: Optional[NovaMultimodalEmbedder] = None,
  client: Optional[Any] = None,
  embedding_provider: Optional[str] = None,
  ...
  embedder = embedder or NovaMultimodalEmbedder()
...
def build_arg_parser() -> argparse.ArgumentParser:
  parser = argparse.ArgumentParser(description="Search CAIPE pre-embedded images with a live query image")
  parser.add_argument("--inspect", action="store_true", help="Only inspect the image collection")
  parser.add_argument("--image", help="Local PNG/JPG/GIF/WebP image to embed and search with")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
  ...
  if not args.inspect and not args.image:
    parser.error("Provide --inspect or --image")

  info = inspect_image_collection(args.milvus_uri, args.collection_name)
  if args.inspect and not args.image:
    if args.json:
